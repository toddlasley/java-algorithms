// Given an array of integers nums, calculate the pivot index of this array.
// The pivot index is the index where the sum of all the numbers strictly to the
// left of the index is equal to the sum of all the numbers strictly to the index's right.

// If the index is on the left edge of the array, then the left sum is 0
// because there are no elements to the left. This also applies to the
// right edge of the array.

// Return the leftmost pivot index. If no such index exists, return -1.

// https://leetcode.com/problems/find-pivot-index/

package me.toddlasley.questions.leetcode.easy.arraysandstrings;

import java.util.List;

public class FindPivotIndex {
    public static int findPivot(final List<Integer> nums) {
        // get the totalSum of array
        // loop through the array, while adding to leftSum
            // check where leftSum == totalSum - leftSum - currentValue
        final int totalSum = nums.stream().reduce(0, Integer::sum);
        int leftSum = 0;

        for (int i = 0; i < nums.size(); i++) {
            int currentValue = nums.get(i);

            if (leftSum == totalSum - leftSum - currentValue) {
                return i;
            }

            leftSum += currentValue;
        }

        return -1;
    }
}
